...
export class ViewModeControl implements IControl {
  ...
  public handleToggleViewMode() {
    const button: HTMLElement = document.querySelector(`#${this.containerId} > div`)!
    ...
    for (const key in this.map?.style._layers) {
      const layer = this.map?.style._layers[key]
      if (layer.type == 'fill-extrusion') {
        if (this._is2DView) {
          this.map.setPaintProperty(layer.id, 'fill-extrusion-height', 0)
        } else {
          this.map.setPaintProperty(layer.id, 'fill-extrusion-height', ['get', 'height'])
        }
      }
    }
  }
